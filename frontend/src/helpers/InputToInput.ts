export default function InputToInput(input: Record<string, number>) {
  const averageMembersPerTeam =
    input.total_project_members / input.number_of_different_teams;
  const averageStoryPoints =
    input.total_story_points / input.total_project_stories;
  const averageStoryPointsPerEpic =
    input.total_story_points / input.number_of_epics;
  const averageStoryPointsPerEngineer =
    input.total_story_points / input.total_project_members;
  const averageSeniorityLevelInDays =
    input.average_seniority_level_per_engineer_in_years * 365;
  ...
  return {
    ...
    estimated_project_duration_in_days:
      input.estimated_project_duration_in_days,
    ...
    average_time_of_story_completion_in_hours:
      input.average_time_of_story_completion_in_hours,
    average_time_of_stories_in_progress_in_hours:
      input.average_time_of_stories_in_progress_in_hours,
  };
}
